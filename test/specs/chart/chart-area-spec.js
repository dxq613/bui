BUI.use(['bui/chart/chart'],function (Chart) {
  ...
  chart.render();
  var series = chart.get('seriesGroup').getFirst();
  describe('测试图形的基础内容',function(){
    it('测试控件生成',function(){
      expect(chart.get('el')).not.toBe(null);
    });

    it('测试area生成',function(){
...
BUI.use(['bui/chart/chart'],function (Chart) {
  ...
  describe('测试图形的基础内容',function(){
    ...
    it('测试起始null的节点',function(){
        var series = seriesGroup.getFirst();
        var points = series.getPoints();
        expect(series.get('data').length).toBe(points.length);
        expect(points[0].value).toBe(null);
    });

    it('测试结束null的节点',function(){

    });

    it('测试中间null的节点',function(){
